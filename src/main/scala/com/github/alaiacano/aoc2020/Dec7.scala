...
object Dec7 extends App {
  ...
  def parseLine(line: String): Map[String, Seq[Container]] = {
    ...
    allContents
      .split(", ") // Split into list of bag contents
      .map { bag: String =>
        // Regex splits it into the color and the count
        val matchOpt = contentRegex.findFirstMatchIn(bag)
  ...
  def numberThatCanContain(
      color: String,
      mapping: Map[String, Seq[Container]]
  ): Set[String] = {
    mapping.get(color) match {
      case Some(listOfContainers) =>
        listOfContainers.map(_.bagColor).toSet ++ listOfContainers
          .map(c => numberThatCanContain(c.bagColor, mapping))
          .reduce(_ + _)
      case None =>
        Set.empty[String]
    }
  ...
  println(
    s"Demo number containing at least one shiny gold: ${numberThatCanContain("shiny gold", demoContainerMapping)}"
  )
  val actual = numberThatCanContain("shiny gold", containerMapping)
  println(
