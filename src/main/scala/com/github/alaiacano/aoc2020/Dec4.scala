...
object Dec4 extends App {

  def checkYear(year: String, minYear: Int, maxYear: Int): Boolean = {
    year.length == 4 && year.toInt >= minYear && year.toInt <= maxYear
  }

  def validHeightStr(h: String): Boolean =
    "^[0-9]{2,3}(cm|in)$".r.findFirstMatchIn(h).nonEmpty
  ...
  def validate(key: String, value: String): Boolean = {
    ...
    key match {
      ...
      case "hgt" if validHeightStr(value) && value.endsWith("cm") =>
        val l = value.dropRight(2).toInt
        l >= 150 && l <= 193
      case "hgt" if validHeightStr(value) && value.endsWith("in") =>
        val l = value.dropRight(2).toInt
        l >= 59 && l <= 76
      case "hcl" => "^#[0-9a-f]{6}$".r.findFirstMatchIn(value).nonEmpty
      case "ecl" => eyeColors.contains(value)
      case "pid" => "^[0-9]{9}$".r.findFirstMatchIn(value).nonEmpty
      case _     => false
    }
  }

  def parseSinglePassport(line: String): MMap[String, String] = {
    val passports = MMap.empty[String, String]
    line
      .split("[ \n]")
      .map(kv => kv.trim().split(":").toList)
      .foreach {
        case k :: v :: Nil => passports(k) = v
        case x             => println(s"WTF IS THIS: ${x}")
      }
    passports
  }

  def validPassport(line: String, doValidation: Boolean): Boolean = {
    val passport: MMap[String, String] = parseSinglePassport(line)
    val requiredKeys = Set("byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid")
    val validKeys = passport
      .filter { case (k, _) => k != "cid" }
      .filter { case (k, v) =>
        if (doValidation) { validate(k, v) }
        else true
      }
      .keySet
  ...
  def numValidPassports(input: String, doValidation: Boolean): Int = {
    val passportLines = input.split("\n\n")
    println(s"NUMBER OF INPUTS: ${passportLines.size}")
    passportLines
      .filter(l => validPassport(l, doValidation))
      .size
  }

  println(
    s"Num valid keys in demo: ${numValidPassports(Dec4Input.demoInput, false)}"
  )
  println(
    s"Num valid keys in input: ${numValidPassports(Dec4Input.input, false)}"
  )

  println(
    s"Num valid keys/values in demo: ${numValidPassports(Dec4Input.demoInput, true)}"
  )
  println(
    s"Num valid keys/values in input: ${numValidPassports(Dec4Input.input, true)}"
  )
}
...
iyr:2011 ecl:brn hgt:59in"""
  ...
  val input = """eyr:2028 iyr:2016 byr:1995 ecl:oth
