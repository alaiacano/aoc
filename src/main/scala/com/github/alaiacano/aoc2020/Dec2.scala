...
object Dec2 extends App {
  ...
  println(s"First way: ${Inputs.passwords.filter(isValidRow).length}")
  println(s"Second way: ${Inputs.passwords.filter(isValidRow2).length}")

  def isValidRow(row: String): Boolean = {
    val (min, max, letter, password) = parseRow(row)
    val cnt = password.count(p => p.toString == letter)
    cnt >= min && cnt <= max
  }

  def isValidRow2(row: String): Boolean = {
    val (first, second, letter, password) = parseRow(row)
    if (password.size < second - 1) {
      false
    } else {
      (password(first - 1).toString == letter) != (password(
        second - 1
      ).toString == letter)
    }
  }

  def parseRow(row: String): (Int, Int, String, String) = {
    val minmax :: letterColon :: password :: Nil = row.split(" ").toList
    val min :: max :: Nil = minmax.split("-").toList
    val letter = letterColon.dropRight(1)
    (min.toInt, max.toInt, letter, password)
  }

}

object Inputs {
  val demoPasswords = List(
    "1-3 a: abcde",
    "1-3 b: cdefg",
    "2-9 c: ccccccccc"
  )
