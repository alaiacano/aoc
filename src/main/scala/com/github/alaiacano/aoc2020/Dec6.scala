...
object Dec6 extends App {

  /** Helper to turn the input into groups of people.
    */
  def parseGroups(input: String): Seq[Seq[String]] = {
    input.split("\n\n").toSeq.map(_.split("[ \n]"))
  }
  ...
  println(s"Part 1 Demo input: ${part1(Dec6Input.demoInput)}")
  println(s"Part 1 Full input: ${part1(Dec6Input.input)}")

  def part2(input: String): Int = {
    parseGroups(input).map { group =>
      // for each group, keep a map of answer -> # of people who had that answer
      val answers = collection.mutable.Map.empty[Char, Int]
      group.foreach { person => 
        person.foreach { letter =>
          answers(letter) = answers.getOrElse(letter, 0) + 1
        }
      }
      answers
        // only keep the answers where ALL people responded
        .filter { case (_, v) => v == group.size }
        .size
    }
    .sum 
  }

  println(s"Part 2 Demo input: ${part2(Dec6Input.demoInput)}")
  println(s"Part 2 Full input: ${part2(Dec6Input.input)}")
}
